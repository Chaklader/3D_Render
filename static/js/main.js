...
const paramsForEachRoom = [
    ...
    {
        rotation: new THREE.Euler(4.35, 0.8, 0.51),
    }
];
...
function changeRoom() {
    ...
    viewer = new Viewer({
        // cameraUp: [-1, 0, -0.1],
        initialCameraPosition: [27.070498222943467, 0.00312424432213776, -2.0],
        initialCameraLookAt: [0, 0, 0],
        sceneRevealMode: 1,
        gpuAcceleratedSort: true,
        enableSIMDInSort: true,
...
// This will change both the Splat URL and also the Euler Matrix
const ROOM_INDEX = 2;
...
async function loadCubeSpaceModel() {
    ...
    const splashLoadingWrapper = document.getElementById(
        "splash-loading-wrapper"
    );

    try {

        const blob = await fetchModelData(rooms[ROOM_INDEX]);
